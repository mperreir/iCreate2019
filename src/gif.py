...
class AnimatedGif(tk.Label):
	...
	def _animate(self):
		...
		try:
			...
			self._num += 1
		except tk.TclError as e :  # When we try a frame that doesn't exist, we know we have to start over from zero
			self._num = 0
			print(e.args)
		# else:
		# self._num = (self._num+1) % 76
...
lbl_with_my_gif = AnimatedGif(root, 'feuille4.gif', 0.04)  # (tkinter.parent, filename, delay between frames)
